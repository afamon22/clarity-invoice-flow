...
export const EditInvoiceForm = ({ isOpen, onClose, invoice }: EditInvoiceFormProps) => {
  ...
  useEffect(() => {
    if (invoice && isOpen) {
      setClientName(invoice.clients?.nom || '');
      setClientEmail(invoice.clients?.email || '');
      setInvoiceNumber(invoice.numero_facture);
      setDueDate(invoice.date_echeance);
      setProvince(invoice.province || 'QC');
      
      if (invoice.items && invoice.items.length > 0) {
        setLineItems(invoice.items.map((item: any, index: number) => ({
        ...
        setLineItems([
          { id: '1', description: 'Service', quantity: 1, price: 0 }
        ]);
      }
    }
  }, [invoice, isOpen]);
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-gray-900">
            Modifier la facture #{invoice?.numero_facture}
          </DialogTitle>
        </DialogHeader>
